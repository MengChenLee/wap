    ...
    <transition enter-active-class="fadeInRight" leave-active-class="fadeOutRight">
      <div v-show="socialShow" class="socialFilter animated">
        ...
        <div class="socialContent">
          ...
          <div class="ageRange">
            <h1>年龄区间</h1>
            <div class="ageInputBox" @click="clickRange(1)">
              <span>{{selectionConPro01.content}}</span>
              <span>岁</span>
            </div>
            <div class="ageRangeSelect" v-show="ageRangeShow">
              <ul>
                <li class="selection" v-for="(item, index) in selectionCon1">
                  <span class="selectionCon" @click="clickSelect(index,1)">{{item.content}}</span>
                </li>
              </ul>
            </div>
          </div>
          <div class="heightRange">
            <h1>身高区间</h1>
            <div class="heightInputBox" @click="clickRange(2)">
              <span>{{selectionConPro02.content}}</span>
              <span>cm</span>
            </div>
            <div class="heightRangeSelect" v-show="heightRangeShow">
              <ul>
                <li class="selection" v-for="(item, index) in selectionCon2">
                  <span class="selectionCon" @click="clickSelect(index,2)">{{item.content}}</span>
                </li>
              </ul>
            </div>
          </div>
          <div class="weightRange">
            <h1>体重区间</h1>
            <div class="weightInputBox" @click="clickRange(3)">
              <span>{{selectionConPro03.content}}</span>
              <span>kg</span>
            </div>
            <div class="weightRangeSelect" v-show="weightRangeShow">
              <ul>
                <li class="selection" v-for="(item, index) in selectionCon3">
                  <span class="selectionCon" @click="clickSelect(index,3)">{{item.content}}</span>
                </li>
              </ul>
            </div>
          </div>
  ...
  export default {
    ...
    data(){
      return {
        //筛选模态框的控制
        socialShow:false,
        heightRangeShow:false,
        weightRangeShow:false,
        ageRangeShow:false,
        selectionConPro01:{
          content:'全部',
          older:0
    ...
    mounted(){
      sessionStorage.setItem("selectionCon1", JSON.stringify(this.selectionCon1));
      sessionStorage.setItem("selectionCon2", JSON.stringify(this.selectionCon2));
      sessionStorage.setItem("selectionCon3", JSON.stringify(this.selectionCon3));
    },
    computed:{
      range(){
        return this.selectionCon.sort((a, b) => a.older - b.older);//升序
      }
    },
    methods: {
      clickRange(num){
        if(num===1){
          this.ageRangeShow=!this.ageRangeShow;
          this.heightRangeShow=false;
          this.weightRangeShow=false;
        }else if(num===2){
          this.heightRangeShow=!this.heightRangeShow;
          this.ageRangeShow=false;
          this.weightRangeShow=false;
        }else{
          this.weightRangeShow=!this.weightRangeShow;
          this.ageRangeShow=false;
          this.heightRangeShow=false;
        }
      },
      clickSelect(index,num){
        if(num===1){
          this.selectionConPro=this.selectionConPro01;
          this.selectionCon=this.selectionCon1;
        }else if(num===2){
          this.selectionConPro=this.selectionConPro02;
          this.selectionCon=this.selectionCon2;
        }else{
          this.selectionConPro=this.selectionConPro03;
          this.selectionCon=this.selectionCon3;
        }
        sessionStorage.setItem("selectionConPro", JSON.stringify(this.selectionConPro));
        this.selectionConP=JSON.parse(sessionStorage.getItem('selectionConPro'));
        if(this.selectionConP.content=='全部'){
          this.selectionConPro.content=this.selectionCon.slice(index,index+1)[0].content;
          this.selectionConPro.older=this.selectionCon.slice(index,index+1)[0].older;
          if(index==0){
            this.selectionCon.splice(0, 1, this.selectionConP);
          }else{
            this.selectionConOld=this.selectionCon.slice(0,1)[0];
            this.selectionCon.splice(0, 1, this.selectionConP);
            this.selectionCon.splice(index, 1, this.selectionConOld);
          }
        }else{
          this.selectionConPro.content=this.selectionCon.slice(index,index+1)[0].content;
          this.selectionConPro.older=this.selectionCon.slice(index,index+1)[0].older;
          this.selectionConOld=this.selectionCon.slice(index,index+1)[0];
          this.selectionCon.splice(index, 1, this.selectionConP);
        }
        // this.range();
        this.ageRangeShow=false;
        this.heightRangeShow=false;
        this.weightRangeShow=false;
        this.selectionCon.sort((a, b) => a.older - b.older);
      },
      btn(index){  //当点击时候点亮，同级的span标签删除Class .splice(index, 0, item);$(event.currentTarget)
        this.ageRangeShow=false;
        this.heightRangeShow=false;
        this.weightRangeShow=false;
      ...
      resetSocial(){
        ...
        this.heightRangeShow=false;
        this.weightRangeShow=false;
        this.selectionConPro01={
          content:'全部',
          older:0
        };
        this.selectionConPro02={
          content:'全部',
          older:0
        };
        this.selectionConPro03={
          content:'全部',
          older:0
        };
        this.selectionCon1=JSON.parse(sessionStorage.getItem('selectionCon1'));
        this.selectionCon2=JSON.parse(sessionStorage.getItem('selectionCon2'));
        this.selectionCon3=JSON.parse(sessionStorage.getItem('selectionCon3'));
...
<style scoped>
  ...
  .heightInputBox span:nth-child(1),
  .weightInputBox span:nth-child(1),
